...
## Transaction-level FIFO provenance

- Added evidence-backed lot propagation across complete address histories.
- Added global edge-capacity protection for converging lots.
- Added safe amount and timezone validation gates.
- Added a reusable live-validation command that records Provider call counts,
  pagination completeness, conservation and transaction-level paths without
...
- 完成 TronGrid 5 地址／三層 bounded Provider-to-engine 驗收；結果依分頁與
  查詢上限正確標記 partial。
- 新增每節點分支控制與未標籤終端候選，保留可重現排序及候選限制。
- 重寫多層追蹤報告組合器，加入層級摘要、交易級主線、FIFO、分流／合流／
  回流、端點候選、後續建議及技術證據附錄。
- 修正多層 Address Registry 將交易雜湊誤判為地址的問題。
- 四格式多層報告共用正確產品副標題，正式正文不再輸出工程亂碼或重複停止條件。
