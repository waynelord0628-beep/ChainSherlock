...
## 17. V8 Desktop Visual Final Validation

- 基準：`6e9871551d98f6d891daa9b37620e7f0d2f84fc2`。
- Blockscout／Blockstream readiness 改為 supported 冷藍語意，明示尚未驗證
  或測試連線；未設定 credential 仍為中性狀態。
- Investigation Queue empty state 將標題、說明與操作收斂為同一置中群組。
- 英文輔助標籤調整為 11px；LIVE EXECUTION 長文字限制高度並保留 tooltip。
- UI targeted：266 passed。
- 完整 pytest：1,213 passed、1 skipped；`pip check` 無相依性錯誤。
- Windows 系統 DPI 未實際切換。AutoGenerated PySide 視窗未能由驗收工具取得
  有效 process id，因此 20 項案件情境僅有 fixture/regression 覆蓋，未宣稱
  全部人工逐頁操作完成。
